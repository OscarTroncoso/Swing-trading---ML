...
def load_v4_params(path: str | Path | None = None, **overrides):
    ...
    data = {
        ...
        'max_holding_bars': 0,
        'thesis_exit_enabled': st.get('thesisExitEnabled', True),
        'exit_policy': st.get('exitPolicy', 'adaptive_checkpoint'),
        'checkpoint_bars': st.get('checkpointBars', 5),
        'continuation_min_trend_score': st.get('continuationMinTrendScore', 3),
        'thesis_exit_rsi_mid': st.get('thesisExitRSIMid', 50.0),
        'thesis_exit_strong_reversal_adx': st.get('thesisExitStrongReversalADX', 25.0),
        'thesis_exit_max_alignment_score': st.get('thesisExitMaxAlignmentScore', 1),
