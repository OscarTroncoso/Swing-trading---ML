...
def test_conditional_extension_has_no_fixed_time_exit():
    p=load_params("config.json"); base=load_v33_policy("config.json")
    pol=replace(base,max_holding_bars=0,stagnation_bars=0,extension_review_bars=1,
                extension_min_mfe_r=100.0,extension_min_close_r=100.0,extension_min_confirmations=3)
    b=backtest_v33(synthetic(),p,pol,start="2022-01-01")
    assert all(t["reason"]!="TIME" for t in b["trades"])
    if b["trades"]:
        assert any(t["reason"]=="DYNAMIC_REVIEW" for t in b["trades"])
